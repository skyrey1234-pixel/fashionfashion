...
export async function generateBuildPlan({ description, garment_type, fit, fabric, target_size, measurements }) {
  ...
  const prompt = `You are an expert patternmaker and garment technologist. Create a practical, realistic build plan for this garment.
...
5. steps — the correct construction order (8–16 steps, e.g. interfacing → shoulder seams → facing → sleeves → side seams → pockets → closures → lining → hem → press → inspect). For each: what to do, stitch type, seam allowance, machine setting, and needle/thread recommendation where relevant.
6. difficulty — a 1–10 construction difficulty score with reasons, required skill level, and estimated sewing hours.
